...
export function ImpactChart({ records, definitions }: ImpactChartProps) {
  // Transformar datos limpios de dominio a formato de serie de tiempo agrupado para Recharts
  const data = records.reduce((acc: any[], record) => {
    ...
    // Acumulamos el valor para la métrica correspondiente en ese día
  ...
  if (data.length === 0) {
    return <div className="h-[400px] flex items-center justify-center text-slate-400">Sin datos registrados</div>;
  }
