...
export function ImportWizard() {
  ...
  const TYPE_OPTIONS: { value: ImportType; label: string; columns: string }[] = [
    { value: "volunteers", label: t("typeVolunteers"), columns: "nombre · apellido · email · telefono · habilidades · fecha_registro" },
    { value: "projects",   label: t("typeProjects"),   columns: "nombre · descripcion · ubicacion · presupuesto · estado · fecha_inicio · fecha_fin" },
    { value: "donations",  label: t("typeDonations"),  columns: "nombre_donante · email · monto · fecha · proyecto · notas" },
  ];

  const STEP_LABELS = [t("stepUpload"), t("stepPreview"), t("stepResult")];
  ...
  async function handleFile(file: File) {
    ...
    if (!res.success) { setError(res.error); setStep("upload"); return; }
  ...
  async function handleConfirm() {
    ...
    if (!res.success) { setError(res.error); setStep("preview"); return; }
    setImportResult(res.data);
    setStep("done");
  }

  function handleReset() {
    setStep("upload"); setParseResult(null); setImportResult(null); setError("");
      ...
      {step === "upload" && (
        <div className="space-y-6">
            ...
            {TYPE_OPTIONS.map((opt) => (
              <button
                ...
                className={`p-4 rounded-xl border-2 text-left transition-all ${
                  type === opt.value ? "border-emerald-500 bg-emerald-50" : "border-slate-200 hover:border-emerald-300 bg-white"
                }`}
              >
          ...
          >
            ...
            <p className="text-slate-700 font-semibold">{t("dropTitle")}</p>
            <p className="text-slate-400 text-sm mt-1">{t("dropSubtitle")}</p>
            <p className="text-xs text-slate-400 mt-3">{t("dropLimits")}</p>
            <input
              ref={fileRef}
              type="file"
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {t("confirmButton", { count: parseResult.totalRows })}
            </button>
          </div>
        </div>
      )}
